...
console.log(`🚀 Starting Barbara Kruger book cover search`);
...
// Search APIs for Barbara Kruger books
async function searchForKrugerCovers() {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (authorLast.toLowerCase().includes('kruger') || 
            authorFull.toLowerCase().includes('barbara kruger')) {
    ...
    console.log(`📚 Found ${krugerBooks.length} Barbara Kruger books\n`);
    
    let successful = 0;
    let failed = 0;
    
    for (const book of krugerBooks) {
        ...
        const filename = `Kruger_Barbara-${book.title.replace(/[^a-zA-Z0-9]/g, '_')}_${book.isbn || book.id}.jpg`;
        ...
        try {
            ...
            if (!coverUrl) {
                try {
                    const query = encodeURIComponent(`"Barbara Kruger" "${book.title}"`);
            ...
            if (!coverUrl) {
                try {
                    const query = encodeURIComponent(`"Barbara Kruger" AND "${book.title}"`);
    ...
    console.log(`\n🎉 Barbara Kruger Cover Search Complete:`);
    console.log(`   Total books: ${krugerBooks.length}`);
    console.log(`   Successful: ${successful}`);
    console.log(`   Failed: ${failed}`);
    console.log(`   Success rate: ${Math.round((successful / krugerBooks.length) * 100)}%`);
}

// Run the search
searchForKrugerCovers().catch(console.error);