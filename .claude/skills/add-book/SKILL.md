...
- **Enriching unmapped columns (`bisac`/`lcc`/`num_images`/`featured`/`custom_page_url`).** After the add, edit the row surgically — never `CSVHandler.write` (it re-quotes every empty field and churns all ~1800 rows). Raw-parse with `csv-parse/sync` (arrays, `relax_column_count:true` only — not `CSVHandler.read`, whose `trim`/auto-correct mutate other rows), change just the target cells, then `csv-stringify/sync` the whole array with `{ header:false, quoted:true, quoted_empty:false }` and match the trailing newline. This round-trips byte-for-byte (verified on the full file, incl. multi-line description fields), so `git diff` shows only the cells you touched. Confirm `git diff --numstat` and `npm run test:csv`. **Reading a row back to verify: use `csv-parse/sync` or a fresh `Read` — not `CSVHandler.read`, whose parsed output renders `; ` as ` `, making correctly-punctuated fields look like run-ons (a real false alarm during a notes review).**
- **Ingest mis-splits author names — verify `author_last` after every add.** The `--json` ingest takes the first token of `authors[0].name` as `author_first` and the rest as `author_last`, even when the JSON supplies explicit `last`/`first` fields. Observed (July 2026 collage batch): "Devin Troy Strother" → last "Troy Strother"; "Nadin Maria Rüfenacht" → last "Maria Rüfenacht"; "Sun Yanchu" (family name first) → last "Yanchu". Mononyms (Linder) end up with an empty `author_last`, which breaks the book-page URL slug and sorts first on author-sorted pages. After ingest, check `author_last` and patch surgically if wrong.
- **Re-running the `--json` ingest re-adds the book.** The script is not idempotent — every run appends a new row with a new ID, so never re-run it "just to see the output" of a completed add (observed: a duplicate Remy row from exactly this). Inspect the earlier run's output or the CSV instead; if a duplicate lands, delete that row surgically per the *Enriching unmapped columns* method.
