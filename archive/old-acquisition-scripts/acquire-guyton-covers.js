...
console.log(`🚀 Starting Wade Guyton book cover search`);
...
// Search APIs for Wade Guyton books
async function searchForGuytonCovers() {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (authorLast.toLowerCase().includes('guyton') || 
            authorFull.toLowerCase().includes('wade guyton')) {
    ...
    console.log(`📚 Found ${guytonBooks.length} Wade Guyton books\n`);
    
    let successful = 0;
    let failed = 0;
    
    for (const book of guytonBooks) {
        ...
        const filename = `Guyton_Wade-${book.title.replace(/[^a-zA-Z0-9]/g, '_')}_${book.isbn || book.id}.jpg`;
        ...
        try {
            ...
            if (!coverUrl) {
                try {
                    const query = encodeURIComponent(`"Wade Guyton" "${book.title}"`);
            ...
            if (!coverUrl) {
                try {
                    const query = encodeURIComponent(`"Wade Guyton" AND "${book.title}"`);
    ...
    console.log(`\n🎉 Wade Guyton Cover Search Complete:`);
    console.log(`   Total books: ${guytonBooks.length}`);
    console.log(`   Successful: ${successful}`);
    console.log(`   Failed: ${failed}`);
    console.log(`   Success rate: ${Math.round((successful / guytonBooks.length) * 100)}%`);
}

// Run the search
searchForGuytonCovers().catch(console.error);