...
console.log(`🚀 Starting Richard Prince book cover search`);
...
// Search APIs for Richard Prince books
async function searchForPrinceCovers() {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (authorLast.toLowerCase().includes('prince') && 
            authorFull.toLowerCase().includes('richard')) {
    ...
    console.log(`📚 Found ${princeBooks.length} Richard Prince books\n`);
    
    let successful = 0;
    let failed = 0;
    
    for (const book of princeBooks) {
        ...
        const filename = `Prince_Richard-${book.title.replace(/[^a-zA-Z0-9]/g, '_')}_${book.isbn || book.id}.jpg`;
        ...
        try {
            ...
            if (!coverUrl) {
                try {
                    const query = encodeURIComponent(`"Richard Prince" "${book.title}"`);
            ...
            if (!coverUrl) {
                try {
                    const query = encodeURIComponent(`"Richard Prince" AND "${book.title}"`);
    ...
    console.log(`\n🎉 Richard Prince Cover Search Complete:`);
    console.log(`   Total books: ${princeBooks.length}`);
    console.log(`   Successful: ${successful}`);
    console.log(`   Failed: ${failed}`);
    console.log(`   Success rate: ${Math.round((successful / princeBooks.length) * 100)}%`);
}

// Run the search
searchForPrinceCovers().catch(console.error);